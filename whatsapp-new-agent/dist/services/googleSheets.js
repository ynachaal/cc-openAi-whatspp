...
exports.syncAllSheetHeaders = syncAllSheetHeaders;
exports.initializeSheetsWithSync = initializeSheetsWithSync;
...
const dubaiTime_1 = require("../utils/dubaiTime");
// Global variables that can be updated
let sheets = null;
let apiKeysCache = null;
let sheetFieldsCache = null;
...
async function processBatch(sheetName, messages) {
    ...
    try {
        ...
        for (const message of messages) {
            ...
            if (fields && fields.length > 0) {
                ...
                const timestamp = message.messageTimestamp ? (0, dubaiTime_1.formatDubaiTime)(message.messageTimestamp) : (0, dubaiTime_1.formatDubaiTime)(Date.now());
                ...
                rowData.push(message.data.transaction_type || '', message.data.property_type || '', message.data.type || '', message.data.location || '', message.data.project_name || '', message.data.size_sqft || '', message.data.bedrooms || '', message.data.bathrooms || '', message.data.status || '', message.data.delivery_year || '', message.data.price_type || '', message.data.asking_price || '', message.data.price_aed || '', message.data.available || '', message.data.special_features ? message.data.special_features.join(', ') : '', message.data.contact || '', message.data.note || '', message.data.notes || '', message.data.unprocessable || '', message.data.is_multi || '', message.data.message_type || '');
...
function addToBatch(data, userInfo, sheetName, originalMessage, messageTimestamp) {
...
async function syncAllSheetHeaders() {
...
async function initializeSheetsWithSync() {
    try {
...
async function ensureHeaders(sheetName, forceUpdate = false) {
    const spreadsheetId = await getGoogleSheetId();
...
async function processPropertyArray(dataArray, userInfo, originalMessage, messageTimestamp) {
    ...
    for (const [transactionType, items] of Object.entries(groupedData)) {
        if (items.length > 0) {
            ...
            for (const item of items) {
                promises.push(addToBatch(item, userInfo, sheetName, originalMessage, messageTimestamp));
            }
        }
    }
...
async function sendToGoogleSheet(data, userInfo, originalMessage, messageTimestamp) {
