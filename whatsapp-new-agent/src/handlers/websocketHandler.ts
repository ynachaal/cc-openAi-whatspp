...
import WhatsAppAgent from "../index";
import { WebSocketMessage } from "../types/websocket";
import { DatabaseService } from "../services/database";
import { syncAllSheetHeaders, clearCache, updateGoogleSheetId } from "../services/googleSheets";
...
export class WebSocketHandler {
  ...
  private isStartupInitialized: boolean = false;
  ...
  private setupAgentCallbacks(): void {
    ...
    this.whatsappAgent.onQRCode((qr: string, renderedQrCode?: string) => {
  ...
  public async initializeAgentOnStartup(): Promise<void> {
  ...
  public async forceReAuthentication(): Promise<void> {
    console.log("🔄 Forcing re-authentication...");
    this.isInitialized = false;
    await this.initializeAgent();
  }

  private validateOrigin(_origin: string | undefined): boolean {
  ...
  private async handleSetWhatsAppNumber(ws: WebSocket, data: any): Promise<void> {
    try {
  ...
  private async handleSetWhatsAppNumbers(ws: WebSocket, data: any): Promise<void> {
    try {
    ...
    try {
      ...
      if (apiKeys?.googleSheetId) {
        try {
          ...
          sheetSyncResult = await updateGoogleSheetId(apiKeys.googleSheetId);
        } catch (syncError) {
  ...
  private async handleGetChatMessagesByDate(ws: WebSocket, _data: any): Promise<void> {
    const isConnected = await this.whatsappAgent.isConnected();
    if (!isConnected) {
      this.sendMessage(ws, {
  ...
  private async handleRefreshWhatsAppNumbers(ws: WebSocket): Promise<void> {
    try {
  ...
  private async handleGetWhatsAppNumbers(ws: WebSocket): Promise<void> {
    try {
      const currentNumbers = this.whatsappAgent.getWhatsAppNumbers();
  ...
  private async handleSyncSheetColumns(ws: WebSocket): Promise<void> {
    try {
