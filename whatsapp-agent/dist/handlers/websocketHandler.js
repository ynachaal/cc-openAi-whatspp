...
class WebSocketHandler {
    ...
    async handleRefreshApiKeys(ws) {
        try {
            const app = app_1.WhatsAppAgentApp.getInstance();
    ...
    async handleGetChatMessagesByDate(ws, data) {
        const whatsappConnected = await this.whatsappService.isConnected();
        if (!whatsappConnected) {
            this.sendMessage(ws, {
        ...
        try {
            ...
            const messages = await this.whatsappService.getAllMessagesDate(whatsappNumber, messageType, formattedDate, formattedTime, limit);
            console.log("messages", whatsappNumber, messageType, formattedDate, formattedTime, limit);
            this.sendMessage(ws, {
                type: "chat_messages_response",
                status: "success",
                data: messages,
