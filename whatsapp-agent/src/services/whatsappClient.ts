...
import { ConnectionStatus, WhatsAppMessage } from "../types/whatsapp";
...
export class WhatsAppClientService {
  ...
  sendQrCodeToWebSocket(ws: any): void {
  ...
  changeIsInitializing(isInitializing: boolean): void {
    this.isInitializing = isInitializing;
  }

  async initialize(ws?: any): Promise<boolean> {
    ...
    try {
      ...
      console.log("Initializing WhatsApp client...", CONFIG);
      ...
      this.client = await create(
        ...
        (
          ...
          if (ws) {
            ...
            ws.send(
              JSON.stringify({ type: "qr_code", data: { qrCode: null } })
            );
        ...
        { updatesLog: true, ...CONFIG.BROWSER_CONFIG }
      );
      ...
      console.error(
        "Error initializing WhatsApp client, trying again...",
        error
      );
  ...
  > {
    ...
    console.log("1111111111", this.client)
    console.log("1111111111", this.client.getAllChats)
    ...
    const unreadMessages =
      (await this.client.getUnreadMessages()) as WhatsAppMessage[];
    const groupMessages = unreadMessages.filter(
      (chat) => chat.isGroupMsg && !chat.groupInfo?.isUser
    );
    ...
    for (const msg of groupMessages) {
      ...
      let chatId =
        msg.chatId?._serialized || msg.from || (msg.id as any)._serialized;
      let chatName = (msg as any).groupInfo?.name || msg.chat?.name || chatId;
  ...
  async getAllMessagesDate(
    number: string,
    type: "higherThan" | "equal" | "lowerThan" | "full",
    date: string,
    time?: string,
    limit?: number
  ): Promise<Message[]> {
    ...
    return await this.client.getAllMessagesDate(
      number,
      type,
      date,
      time!,
  ...
  async analyzeMessagesSinceLastDate(): Promise<void> {
    if (!this.client || this.whatsappNumbers.length === 0) {
      console.log(
        "❌ Cannot analyze messages: WhatsApp client or numbers not available"
      );
      return;
    }
    ...
    try {
      ...
      if (!lastAnalyzedDate) {
        console.log(
          "📅 No last analyzed date found, skipping historical analysis"
        );
        return;
      }
      ...
      console.log(
        `📊 Last analyzed: ${lastAnalyzedDate}, Now: ${now.toISOString()}, Days difference: ${daysDiff}`
      );
      console.log(
        `🎯 Will fetch messages starting from: ${nextSecondDate.toISOString()} (last analyzed + 1 second)`
      );
      ...
      console.log(
        `🔄 Fetching messages since ${formattedDate} ${formattedTime} from ${this.whatsappNumbers.length} groups`
      );
      ...
      for (const whatsappNumber of this.whatsappNumbers) {
        try {
          console.log(
            `📞 Fetching messages from group: ${whatsappNumber} since ${formattedDate} ${formattedTime}`
          );
          const messages = await this.getAllMessagesDate(
            whatsappNumber,
            "higherThan",
            formattedDate,
            formattedTime,
          ...
          if (messages && messages.length > 0) {
            ...
            console.log(
              `📨 Found ${messages.length} messages from ${whatsappNumber}`
            );
          }
        } catch (error) {
          console.error(
            `❌ Error fetching messages from ${whatsappNumber}:`,
            error
          );
        }
      }
      ...
      console.log(
        `📨 Found ${filteredMessages.length} new messages to analyze from all groups`
      );
      ...
      for (let batchIndex = 0; batchIndex < messageBatches.length; batchIndex++) {
        ...
        const batchPromises = batch.map(async (message) => {
          try {
            const timestampToSave: Date | null =
              await this.messageProcessor.processHistoricalMessage(
                message,
                new Date(lastAnalyzedDate)
              );
      ...
      if (latestTimestamp && lastProcessedId) {
        console.log(
          `📅 Updating last analyzed message date to: ${(latestTimestamp as Date).toISOString()}`
        );
      ...
      console.log(
        `✅ Historical analysis complete: ${processedCount} messages processed, ${realEstateCount} real estate messages found`
      );
    } catch (error) {
  ...
  async forceKillAllChromeProcesses(): Promise<void> {
  ...
  async isReadyForInitialization(): Promise<boolean> {
    // Check if there are any lingering session files
    const tokensPath = path.join(process.cwd(), "tokens");
    const sessionPath = path.join(
      process.cwd(),
      `${CONFIG.WHATSAPP_SESSION_NAME}.json`
    );

    if (fs.existsSync(tokensPath) || fs.existsSync(sessionPath)) {
    ...
    try {
      ...
      if (chromeProcessCount > 0) {
        console.log(
          `⚠️ Found ${chromeProcessCount} Chrome processes, cleaning them up...`
        );
  ...
  private async killChromeProcessesWindows(): Promise<void> {
        ...
        if (parts.length >= 2) {
          ...
          if (pid && pid !== "PID" && !isNaN(Number(pid))) {
            try {
              ...
              console.log(
                `⚠️ Could not kill Chrome process ${pid}:`,
                killError
              );
            }
          }
  ...
  private async killChromeProcessesLinux(): Promise<void> {
    ...
    for (const processName of chromeProcessNames) {
      try {
        ...
        const { stdout } = await execAsync(`pgrep -f ${processName}`).catch(
          () => ({ stdout: "" })
        );
        ...
        if (stdout.trim()) {
          const pids = stdout
            ...
            .filter((pid) => pid && !isNaN(Number(pid)));
          console.log(
            `📋 Found ${pids.length} ${processName} processes: ${pids.join(
              ", "
            )}`
          );
      ...
      try {
        ...
        if (stdout.trim()) {
          const pids = stdout
            ...
            .filter((pid) => pid && !isNaN(Number(pid)));
          console.log(
            `💀 Force killing remaining ${processName} processes: ${pids.join(
              ", "
            )}`
          );
    ...
    try {
      console.log(
        "🧹 Final cleanup - killing any remaining chrome-related processes..."
      );
   ...
   */
  async clickContinueButtonIfExists(): Promise<void> {
    try {
      if (!this.client || !this.client.waPage) {
        console.log(
          "❌ Cannot click Continue button: WhatsApp page not available"
        );
        return;
      }

      console.log(
        "⏳ Waiting 5 seconds before checking for Continue button..."
      );
      ...
      try {
        ...
        console.log(
          "ℹ️ Text-based selector didn't work, trying alternatives..."
        );
      }
      ...
      if (!buttonFound) {
        try {
          ...
          console.log(
            "⚠️ XPath selector method failed:",
            error instanceof Error ? error.message : String(error)
          );
        }
      }
      ...
      if (!buttonFound) {
        console.log(
          "ℹ️ Continue button not found - this is normal if already logged in or button doesn't exist"
        );
        ...
        try {
          ...
          console.log(
            "📸 Screenshot taken for debugging (base64 length):",
            screenshot.length
          );
  ...
  async resetEverything() {
    ...
    try {
      ...
      const sessionFiles = [
        path.join(process.cwd(), `${CONFIG.WHATSAPP_SESSION_NAME}.zip`),
        path.join(process.cwd(), `${CONFIG.WHATSAPP_SESSION_NAME}.crdownload`),
        path.join(process.cwd(), `${CONFIG.WHATSAPP_SESSION_NAME}.tmp`),
      ];

      for (const file of sessionFiles) {
        if (fs.existsSync(file)) {
          fs.unlinkSync(file);
          console.log(`✅ Removed session file: ${path.basename(file)}`);
        }
      }
    } catch (error) {
