...
import { UserInfo, MessageAnalysis, WhatsAppMessage } from "../types/whatsapp";
...
export class MessageProcessor {
  ...
  private extractUserInfo(message: WhatsAppMessage | Message): UserInfo {
    try {
      // Extract phone number from sender.id (remove @c.us suffix if present)
      const phone =
        (message as Message).author?.replace?.("@c.us", "") ||
        (message as WhatsAppMessage).sender?.id?.user?.replace?.("@c.us", "") ||
        "";
      ...
      console.log(`👤 User info - Phone: ${phone}, Name: ${name}`);
      return { phone, name };
    } catch (error) {
  ...
  async processMessage(
    ...
    await sendToGoogleSheet(analysis, userInfo, messageBody);
  ...
  async processHistoricalMessage(
    ...
    await sendToGoogleSheet(analysis, userInfo, message.body);
