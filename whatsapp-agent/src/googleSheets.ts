...
const sheetQueue = new PQueue({ concurrency: 1 });
...
export function updateGoogleSheetId(sheetId: string) {
    if (!sheetId) {
...
async function appendToSheet(data: any, userInfo: { phone: string; name: string }, sheetName: string, originalMessage?: string) {
    const spreadsheetId = await getGoogleSheetId();
    ...
    if (fields && fields.length > 0) {
        ...
        rowData.push(
            ...
            data.price_aed || '',
            data.available || '',
            data.special_features ? data.special_features.join(', ') : '',
            data.contact || '',
            data.note || '',
            data.notes || '',
            data.unprocessable || '',
...
async function processPropertyArray(dataArray: any[], userInfo: { phone: string; name: string }, originalMessage?: string) {
    const spreadsheetId = await getGoogleSheetId();
    ...
    for (const [transactionType, items] of Object.entries(groupedData)) {
        if (items.length > 0) {
            ...
            for (const item of items) {
                promises.push(
                    sheetQueue.add(() => appendToSheet(item, userInfo, sheetName, originalMessage))
                );
            }
        }
    }
...
export async function sendToGoogleSheet(data: any, userInfo: { phone: string; name: string }, originalMessage?: string) {
